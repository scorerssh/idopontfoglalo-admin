...
{
    public abstract class ReservationRoomFilterAttributeBase : FilteringOptionsBaseAttribute
    {
        ...
        protected static MemberExpression GetRoomExpression(ExpressionBuildContext context)
        {
    ...
    public class ReservationInApartmanByIdAttribute : ReservationRoomFilterAttributeBase
    {
        public override Expression BuildExpression(ExpressionBuildContext context)
        {
            var roomExpression = GetRoomExpression(context);
            var apartmanIdProperty = Expression.Property(roomExpression, nameof(Room.ApartmanId));
            var filterValue = Expression.Constant(context.FilterObjectPropertyValue, typeof(int));

            return Expression.Equal(apartmanIdProperty, filterValue);
        }
    }

    public class ReservationInUserByIdAttribute : ReservationRoomFilterAttributeBase
    {
        public override Expression BuildExpression(ExpressionBuildContext context)
        {
            var roomExpression = GetRoomExpression(context);
            var apartmanExpression = Expression.Property(roomExpression, nameof(Room.Apartman));
